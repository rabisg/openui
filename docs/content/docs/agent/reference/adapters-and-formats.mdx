...
| `buildBody`     | `(params) => unknown`      | No       | Replaces the default AG-UI request body; receives canonical messages and their formatter. |
| `onRequestStart` | `() => void`              | No       | Observes request starts.                                                                 |
| `onResponseError` | `(response) => void \| Promise<void>` | No | Observes a clone of non-2xx responses.                                        |

**Request body.** By default, `fetchLLM` POSTs an AG-UI-shaped body with `threadId`, a fresh `runId`, formatted `messages`, and empty `tools` and `context` arrays. `buildBody` can replace this shape. The UI's `AbortSignal` is always forwarded.
...
Use `useLLM(options)` in React when options reference live state. It preserves `ChatLLM` identity across renders while each request reads the latest options:
